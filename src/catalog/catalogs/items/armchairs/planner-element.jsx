...
function makeArmchairMaxLOD() {
    ...
    let g_asse_rotaz2 = new Three.CylinderGeometry(
        0.06,
        0.06,
        0.8,
        32,
...
export default {
    ...
    render2D: function (element, layer, scene) {
        ...
        return (
            <g
                transform={`translate(${
                    ((flip ? -1 : 1) * WIDTH * seats) / 2
                },${-DEPTH / 2}) scale(${flip ? 1 : -1},1)`}
            >
                {seatsArray}
                <text
                    x="0"
    ...
    render3D: function (element, layer, scene) {
        ...
        function setArmchairsPos(listObject, seats) {
            ...
            for (let ind = 0; ind < seats; ind++) {
                seatArray[ind].position.x =
                    (ind - Math.floor(seats / 2)) * -0.8;

                if (flip)
                    seatArray[ind].position.z =
                        (ind - Math.floor(seats / 2)) * -0.085;
                else
                    seatArray[ind].position.z =
                        (ind - Math.floor(seats / 2)) * +0.085;
            }
