...
function makeObjectMaxLOD() {
    ...
    //top
    let top = new Three.Mesh(new Three.BoxGeometry(1, 0.2, 1.5), steelTexture);
    ...
    //central fire
    let centralFire = new Three.Mesh(
    ...
    for (let gx = -0.45; gx <= 0.45; gx += 0.45) {
        if (gx !== 0) {
            let longSideGrid = new Three.Mesh(
            ...
            for (let gz = -0.46; gz <= 0.68; gz += 0.9) {
                let longSideGrid2 = new Three.Mesh(
    ...
    for (let gz = -0.675; gz <= 0.675; gz += 0.45) {
        let shortSideGrid = new Three.Mesh(
