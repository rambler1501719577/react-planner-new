...
export default class ImageFul extends Component {
        ...
        if (target.nodeName === "circle") {
            if (target.attributes.name) {
                if (target.attributes.name.nodeValue === "fst-anchor") {
                    this.setState({
                        handleMouseMove1: !this.state.handleMouseMove1,
                    });
                } else if (target.attributes.name.nodeValue === "snd-anchor") {
                    this.setState({
                        handleMouseMove2: !this.state.handleMouseMove2,
                    });
                }
            }
        }
    ...
    onMouseMove(event) {
        ...
        let scale =
            !isNaN(dist) && dist ? this.props.distance.length / dist : 0;
    ...
    componentWillUnmount() {
        document.removeEventListener(
            "mousedown-planner-event",
            this.onMouseDown
        );
        document.removeEventListener(
            "mousemove-planner-event",
            this.onMouseMove
        );
    }
