...
function makeObjectMaxLOD() {
    ...
    for (let fbx = -4.5; fbx <= 4.5; fbx += 9) {
        for (let fbz = -9.45; fbz <= 9.45; fbz += 18.9) {
            ...
            if (fbx < 0) {
                support2.position.set(fbx + 3, 1.6, fbz);
                nutBack2.position.set(fbx + 3, 2.5, fbz - 0.2);
                if (fbz < 0) nutBack2.position.set(fbx + 3, 2.5, fbz + 0.2);
            }

            if (fbz < 0) {
                nutBack1.position.set(fbx, 2.5, fbz + 0.2);

                if (fbx > 0) nutBack2.position.set(fbx - 3, 2.5, fbz + 0.2);
            }
        }
    }
    ...
    for (let fbsx = -4.5; fbsx <= 4.5; fbsx += 9) {
        for (let fbsz = -9; fbsz <= 9; fbsz += 18) {
            // seat support
            let seatSupport1 = new Three.Mesh(
            ...
            let seatSupport2 = new Three.Mesh(
                new Three.BoxGeometry(0.25, 0.125, 1),
                grey
            );
            ...
            if (fbsx < 0) {
                seatSupport2.position.set(fbsx + 3, -0.35, fbsz);
                seatNut2.position.set(fbsx + 3, -0.35, fbsz - 2);
                if (fbsz < 0) seatNut2.position.set(fbsx + 3, -0.35, fbsz + 2);
            }

            if (fbsz < 0) {
                seatNut1.position.set(fbsx, -0.35, fbsz + 2);

                if (fbsx > 0) seatNut2.position.set(fbsx - 3, -0.35, fbsz + 2);
            }
        }
    }
...
function makeObjectMinLOD() {
    ...
    for (let fx = -3; fx <= 3; fx += 6) {
        for (let fz = 8; fz >= -8; fz -= 16) {
            ...
            if (fz > 0) legBase.position.set(fx, -3.5, fz - 1);
        }
    }
    ...
    for (let fbx = -4.5; fbx <= 4.5; fbx += 9) {
        for (let fbz = -9.45; fbz <= 9.45; fbz += 18.9) {
            ...
            if (fbx < 0) support2.position.set(fbx + 3, 1.6, fbz);
        }
    }
    ...
    for (let fbsx = -4.5; fbsx <= 4.5; fbsx += 9) {
        for (let fbsz = -9; fbsz <= 9; fbsz += 18) {
            ...
            if (fbsx < 0) seatSupport2.position.set(fbsx + 3, -0.35, fbsz);
        }
    }
...
export default {
    ...
    render3D: function (element, layer, scene) {
        ...
        let valuePosition = new Three.Box3().setFromObject(
            canteen_table_MaxLOD
        );
