...
GLTFLoader = (function () {
  ...
  function GLTFLightsExtension(json) {
    ...
    var extension =
      (json.extensions && json.extensions[EXTENSIONS.KHR_LIGHTS]) || {};
      ...
      switch (light.type) {
        ...
        case "spot":
          ...
          light.spot.innerConeAngle =
            light.spot.innerConeAngle !== undefined
              ? light.spot.innerConeAngle
              : 0;
          light.spot.outerConeAngle =
            light.spot.outerConeAngle !== undefined
              ? light.spot.outerConeAngle
              : Math.PI / 4.0;
          lightNode.angle = light.spot.outerConeAngle;
          lightNode.penumbra =
            1.0 - light.spot.innerConeAngle / light.spot.outerConeAngle;
          lightNode.target.position.set(0, 0, 1);
          lightNode.add(lightNode.target);
          break;
  ...
  GLTFParser.prototype.loadTexture = function (textureIndex) {
    ...
    return Promise.resolve(sourceURI)
      ...
      .then(function (texture) {
        ...
        if (!textureExtensions[EXTENSIONS.MSFT_TEXTURE_DDS]) {
          texture.format =
            textureDef.format !== undefined
              ? WEBGL_TEXTURE_FORMATS[textureDef.format]
              : THREE.RGBAFormat;
        }

        if (
          textureDef.internalFormat !== undefined &&
          texture.format !== WEBGL_TEXTURE_FORMATS[textureDef.internalFormat]
        ) {
          console.warn(
